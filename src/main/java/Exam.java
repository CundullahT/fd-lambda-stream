...
public class Exam {
    ...
    public static List<User> readAllManagers() {
        ...
        return DataGenerator.fillUsers().stream().filter(user -> user.getRole().getName().equals("MANAGER")).collect(Collectors.toList());
    ...
    public static List<User> readAllSuspendedUsers() {
        ...
        return DataGenerator.fillUsers().stream().filter(user -> user.getState() == UserState.SUSPENDED).collect(Collectors.toList());
    ...
    public static Integer sumDurationForAllData() {
        ...
        return DataGenerator.fillCoursesAssigned().stream().map(x -> x.getCourse().getDuration()).reduce(0,Integer::sum);
    ...
    public static List<CourseAssigned> findCoursesByUser() {
        ...
        return DataGenerator.fillCoursesAssigned().stream().filter(courseAssigned -> courseAssigned.getUser().getId().equals(specificUser.getId())).collect(Collectors.toList());
    ...
    public static List<Map<String, Object>> modifyObject() {
        ...
        return DataGenerator.fillCoursesAssigned().stream().filter(course -> course.getStatus() == CourseStatus.FINISHED)
                .map(course -> {
                    Map<String, Object> modifiedResult = new HashMap<>();
                    modifiedResult.put("courseName",course.getCourse().getName());
                    modifiedResult.put("courseDuration",course.getCourse().getDuration());
                    modifiedResult.put("username",course.getUser().getFirstName() + " " + course.getUser().getLastName());
    ...
    public static BigDecimal divideToWeek() {
        ...
        BigDecimal convertToWeek = DataGenerator.fillCoursesAssigned().stream()
                .filter(courseAssigned -> courseAssigned.getStatus() == CourseStatus.FINISHED &&
                        courseAssigned.getUser().equals(specificUser))
